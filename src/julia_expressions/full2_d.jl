function full2_d(u, mu, beta, v, vp, w, k::SVector, kp::SVector, q::SVector, k1::SVector)
    u +
    (u ^ 2 * fermidist(-1mu + disp(k1), beta)) * (w + disp(k1) - disp(k1 + q)) ^ -1 +
    (
    ...
    ((u ^ 2 * fermidist(-1mu + disp(k1 + q), beta)) * (w + disp(k1) - disp(k1 + q)) ^ -1) +
    (
